...
L.HexbinLayer = L.Layer.extend({
  ...
  getFlexRadius() {
    ...
    } if (this.map.getZoom() > 2 && this.map.getZoom() < 8) {
  ...
  getEvents() { return { zoomend: this._zoomChange }; },
  _zoomChange(evt) {
    this._disableLeafletRounding();
  ...
  _redraw(selection, projection, zoom) {
    ...
    const data = this._data.map((d) => {
      ...
      const point = projection.latLngToLayerPoint([lat, lng]);
      return { o: d, point };
    });

    // Select the hex group for the current zoom level. This has
    // the effect of recreating the group if the zoom level has changed
    const join = selection.selectAll('g.hexbin')
      .data([zoom], (d) => d);
  ...
  _createHexagons(g, data, projection) {
    ...
    const join = g.selectAll('path.hexbin-hexagon')
      .data(bins);
